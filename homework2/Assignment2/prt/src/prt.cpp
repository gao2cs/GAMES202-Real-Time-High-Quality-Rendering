...
public:
    ...
    std::unique_ptr<std::vector<double>> computeInterReflectionSH(Eigen::MatrixXf* directTSHCoeffs, const Point3f& v, const Normal3f& n, const Scene* scene, int bounces) {
        ...
        for (int t = 0; t < sample_side; t++) {
            ...
            Intersection its;
                ...
                // 1. Shoot a ray from v in direction of wi
                if (H > 0.0 && scene->rayIntersect(Ray3f(v, wi.normalized()), its)) {
                    ...
                    // Computing the interpolated Light Transport LT SH conefficients
                    
                    // Vertex Interpolated hitNormal
                    ...
                    std::unique_ptr<std::vector<double>> nextBouncesCoeffs = computeInterReflectionSH(directTSHCoeffs, hitPos, hitNormal, scene, bounces + 1);

        ...
        for (int i = 0; i < 6; i++)
        {
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    // Start
                    float delta_wi = CalcArea(x, y, width, height); // dw = dA/r^2
                    for (int l = 0; l <= SHOrder; l += 1) {
                        for (int m = -l; m <= l; m += 1) {
                            double SH_wi = sh::EvalSH(l, m, Eigen::Vector3d(dir.x(), dir.y(), dir.z()).normalized());
                            int index = sh::GetIndex(l, m);
                            SHCoeffiecents[index] += Le * SH_wi * delta_wi;
                        }
                    }
                    // End
                }
...
public:
    ...
    // Start
    std::unique_ptr<std::vector<double>> computeInterreflectionSH(const Point3f& v, const Normal3f& n, const Scene* scene, int bounce) {
        const int sample_side = static_cast<int>(floor(sqrt(m_SampleCount)));
        std::unique_ptr<std::vector<double>> indirCoeffs(new std::vector<double>());
        indirCoeffs->assign(SHCoeffLength, 0.0);
        ...
        //double dw = 4 * M_PI / (sample_side * sample_side); 

        // MC uniform sampling on a unit sphere
        for (int t = 0; t < sample_side; t++) {
            for (int p = 0; p < sample_side; p++) {
                ...
                if (H > 0.0 && scene->rayIntersect(Ray3f(v, wi.normalized()), its)) {
                    ...
                    Point3f tri_index = its.tri_index;
                    Point3f hitPoint = its.p;

                    MatrixXf normals = its.mesh->getVertexNormals();  
                    Normal3f hitNormal = (normals.col(tri_index.x()) * bary.x() + normals.col(tri_index.y()) * bary.y() + normals.col(tri_index.z()) * bary.z()).normalized();

                    std::unique_ptr<std::vector<double>> restSHCoeffs= computeInterreflectionSH(hitPoint, hitNormal, scene, bounce + 1);
                    for (int k = 0; k < SHCoeffLength; k += 1) {
                        
                        float interpCoeffs = m_TransportSHCoeffs.col(tri_index.x()).coeffRef(k) * bary.x() +
                        m_TransportSHCoeffs.col(tri_index.y()).coeffRef(k) * bary.y() +
                        m_TransportSHCoeffs.col(tri_index.z()).coeffRef(k) * bary.z();

                        (*indirCoeffs)[k] += (interpCoeffs + (*restSHCoeffs)[k]) * H;
                    }
                }
            }
        } 

        // MC Approximation = 1/N * \sum_i f(wi)
        for (int k = 0; k < SHCoeffLength; k += 1) {
            (*indirCoeffs)[k] /= sample_side * sample_side;
        }
    ...
    virtual void preprocess(const Scene *scene) override
    {
        ...
        // 2. Light Transport Coefficients
        ...
        for (int i = 0; i < mesh->getVertexCount(); i++)
        {
            ...
            // 2.1 Function for computing the light transport term 
        ...
        if (m_Type == Type::Interreflection) 
        {
            ...
            for (int i = 0; i < mesh->getVertexCount(); i += 1)
            {
                ...
                std::unique_ptr<std::vector<double>> indirSH = computeInterreflectionSH(v, n, scene, 1);

                for (int j = 0; j < SHCoeffLength; j += 1) {
                    m_TransportSHCoeffs.col(i).coeffRef(j) += (*indirSH)[j];
                }
...
private:
    ...
    Eigen::MatrixXf m_TransportSHCoeffs; // 9x|V| matrix
    Eigen::MatrixXf m_LightCoeffs;       // 3x9 matrix
};
